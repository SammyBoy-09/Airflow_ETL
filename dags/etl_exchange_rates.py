...
def load_exchange_rates(**context):
    ...
    with engine.begin() as conn:
        ...
        conn.execute(text("DROP TABLE IF EXISTS etl_output.exchange_rates"))
    ...
    print(f"✅ Loaded {len(df):,} exchange rates to etl_output.exchange_rates")
    
    context['ti'].xcom_push(key='loaded_rows', value=len(df))
    
    return {'rows': len(df), 'table': 'etl_output.exchange_rates'}
